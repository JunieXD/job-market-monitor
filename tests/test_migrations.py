...
def test_city_and_search_read_models_can_downgrade_and_restore() -> None:
    engine = create_engine("sqlite+pysqlite:///:memory:")
    create_schema(engine)
...
def test_city_display_migration_preserves_raw_names_and_repoints_history() -> None:
    ...
    with Session(engine) as session, session.begin():
        ...
        session.flush()
        locations = [
            Location(source_id=source.id, code="CD", name="四川省·成都"),
            Location(source_id=source.id, code="CD-HQ", name="成都总部"),
        ]
