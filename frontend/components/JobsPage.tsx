  ...
  return (
    <>
      ...
      <Panel title="岗位列表" note={`${formatNumber(total)} 个岗位${coverage?.snapshot_date ? ` · 更新至 ${coverage.snapshot_date}` : ""}`}>
        {loading && !result ? <LoadingBlock /> : result?.data.length ? <JobTable rows={result.data} /> : <EmptyState title="没有符合条件的岗位" detail="请调整筛选条件或搜索词。" />}
...
function JobTable({ rows }: { rows: JobRow[] }) {
  return <TableWrap><table className="fit-table jobs-table"><thead><tr><th>岗位</th><th>公司</th><th>招聘类型</th><th>官网更新</th><th>数据更新</th><th><span className="sr-only">官网链接</span></th></tr></thead><tbody>{rows.map((row) => <tr key={`${row.source_key}-${row.external_id}`}><td className="job-title"><strong>{row.title}</strong></td><td>{row.company_name}</td><td><ChannelTag channel={row.channel} /></td><td>{row.source_updated_at ? new Date(row.source_updated_at).toLocaleDateString("zh-CN") : "-"}</td><td>{new Date(row.last_seen_at).toLocaleDateString("zh-CN")}</td><td><a className="external-link" href={row.source_url} target="_blank" rel="noreferrer" aria-label={`在官网查看 ${row.title}`}><ExternalLink size={15} /><span>官网</span></a></td></tr>)}</tbody></table></TableWrap>;
}
