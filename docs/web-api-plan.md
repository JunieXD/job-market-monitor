...
- TanStack Query：请求缓存、加载状态、错误状态和筛选条件联动。
- Tailwind CSS + shadcn/ui：保持信息密度、可扫描和可访问的分析工具风格。
...
3. 公司详情：来源、渠道、分类、城市和生命周期。
4. 分类与城市：排行、占比、矩阵和趋势。
5. 岗位浏览器：筛选、分页、详情和官网链接。
6. 数据质量：来源健康、完整性和字段覆盖率。
...
本文件是当前 Goal 的验收依据。当前先执行阶段一，完成分析契约、查询层边界和测试基线；阶段一
通过后提交 Git，再进入 FastAPI MVP。生产服务器在所有阶段均保持不变。
